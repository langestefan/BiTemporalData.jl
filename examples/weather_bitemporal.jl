...
# is a forecast of `target_date`'s temperature (valid time) as issued on
# `issued_on` (transaction time). The same day is forecast on each of the 7
...
# `correct!` in issue order), so the daily revisions chain in transaction time.
store = load!(
