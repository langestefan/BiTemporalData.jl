@testitem "TimeZones extension stores zoned times as UTC instants" tags = [:unit] begin
    ...
    insert!(
        s, "X", 1.0; valid_from = ZonedDateTime(2024, 1, 1, 12, tz"America/New_York"),
    ...
    @test as_of(
        s, "X"; valid_at = ZonedDateTime(2024, 1, 1, 18, tz"Europe/Berlin"),
        tx_at = DateTime(2024, 2, 1)
    ) == 1.0                                    # 17:00 UTC
    @test as_of(
        s, "X"; valid_at = ZonedDateTime(2024, 1, 1, 11, 59, tz"America/New_York"),
        tx_at = DateTime(2024, 2, 1)
